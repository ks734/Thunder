    ...
    for iface in interfaces:
        ...
        for idx, m in enumerate(emit_methods):
            ...
            def Convert(paramtype):
                ...
                if isinstance(p, CppParser.Integer):
                    if param.IsPointer():
                        ...
                        if p.size == "char" and "signed" not in p.type and "unsigned" not in p.type and "_t" not in p.type:
                            value = "CHAR"
                        else:
                            if p.size == "char":
                                value = "INT8"
                    ...
                    return "Type.ENUM" + signed + value

                elif isinstance(p, CppParser.BuiltinInteger) and INSTANCE_ID in paramtype.type.Type().full_name:
                    return "Type.INSTANCE"
            ...
            if rv:
                ...
                text.append("type = %s" % (rv))

                if isinstance( m.retval.type.Resolve().Type(), CppParser.Enum):
                    text.append("enum = \"%s\"" %  m.retval.type.Type().full_name.replace("::WPEFramework::", ""))
            ...
            for p in m.vars:
                ...
                if param:
                    ...
                    text.append("type = %s" % (param))

                    if isinstance(p.type.Resolve().Type(), CppParser.Enum):
                        text.append("enum = \"%s\"" % p.type.Type().full_name.replace("::WPEFramework::", ""))
